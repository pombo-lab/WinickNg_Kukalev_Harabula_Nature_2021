...
"""
====================================================================================================================
Perl script name Insulation_score_TAD_calling_Crane_etal_adjustedForGAM.pl
The original script was taken from https://github.com/blajoie/crane-nature-2015 and adjusted for usage in GAM data

The modified script is part of the study: Winick-Ng, W., Kukalev, A., Harabula, I., et al. "Cell-type specialization is encoded 
by specific chromatin topologies", Nature 2021

Date: August 16, 2021

This script computes insulation score on GAM chromatin contact matrices
...
use lib "/data/pombo/software/tadcalling_insulator_score";
use cworld::crane_nature2015_adjustedForGAM qw(:all);

sub check_options {
	my $opts = shift;

	my ($inputMatrix,$verbose,$insulationSquareSize,$insulationDeltaSpan,$insulationMode,$noiseThreshold,$boundaryMarginOfError,$ignoreZero,$inputMatrixName);
	...
	# START GAM ADJUSTMENT
	if( exists($opts->{ outputName }) ) {
		$inputMatrixName = $opts->{ outputName };
	} else {
		$inputMatrixName = getFileName($inputMatrix);
	}
	# END GAM ADJUSTMENT

	return($inputMatrix,$verbose,$insulationSquareSize,$insulationDeltaSpan,$insulationMode,$noiseThreshold,$boundaryMarginOfError,$ignoreZero,$inputMatrixName);
}
...
sub help() {
	...
	printf("\n\t%-10s %-10s %-10s\n", "-output", "[inputMatrix]", "Name of output file (default is to generate a file in the cwd with similar name to the input file)");
...
my %options;
my $results = GetOptions( \%options,'inputMatrix|i=s','verbose|v','insulationSquareSize|is=s','insulationDeltaSpan|ids=s','insulationMode|im=s','noiseThreshold|nt=s','boundaryMarginOfError|bmoe=s','ignoreZero|iz','outputName|output=s');

#user Inputs
my ($inputMatrix,$verbose,$insulationSquareSize,$insulationDeltaSpan,$insulationMode,$noiseThreshold,$boundaryMarginOfError,$ignoreZero,$inputMatrixName)=check_options( \%options );
...
print "outputName (-ouput)\t$inputMatrixName\n" if($verbose);
